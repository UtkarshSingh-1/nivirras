...
import { FeaturedCandles } from "./FeaturedCandles";
import { Categories } from "./Categories";
import { About } from "./About";
import { Testimonials } from "./Testimonials";
import { HomeHero } from "./HomeHero";
import { TrendingCandles } from "./TrendingCandles";

function LocalNewsletter() {
    ...
    return (
        <section className="py-24 px-6 bg-gradient-to-b from-[#D3DAAE] to-[#F2F4E8] relative overflow-hidden">
            <motion.div
                ...
                className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-radial from-[#8A9353]/20 to-transparent blur-3xl shadow-[0_0_100px_rgba(138,147,83,0.3)]"
            />
                    ...
                    >
                        <div className="relative">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-2 bg-white/70 backdrop-blur-md rounded-3xl sm:rounded-full shadow-lg border border-white/50 p-3 sm:p-2 hover:shadow-xl transition-shadow">
                                <div className="flex items-center gap-2 w-full min-w-0">
                                    <Mail className="w-5 h-5 text-[#636B2F] ml-2 sm:ml-4 shrink-0" aria-hidden="true" />
                                    <input
                                        type="email"
                                        value={email}
                        ...
                        {isSubmitted && (
                            <motion.div
                                ...
                                style={{ fontFamily: "'Inter', sans-serif" }}
                            >
                                Welcome to the tribe! Check your inbox soon.
                            </motion.div>
                        )}
                    </motion.form>
...
export function UltimateHome({ featuredProducts, trendingProducts }: { featuredProducts: any[], trendingProducts: any[] }) {
    return (
